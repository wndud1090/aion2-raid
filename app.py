...
st.markdown("""
    ...
    .match-gold .has-match { color: #FFD700 !important; font-weight: 900; }
...
def check_8man_match(day_df):
    """8명이 1시간이라도 겹치는 구간이 있는지 판별"""
    if len(day_df) < 8: return False
    
    # 0시부터 47시까지(익일 포함) 타임라인 배열 생성
...
with st.sidebar:
    ...
    with st.expander("👥 대원 명단", expanded=False):
        raw_names = st.text_area("쉼표 구분 입력", "공대장, 대원1, 대원2, 대원3, 대원4, 대원5, 대원6, 대원7")
...
# --- 4. 메인: 달력 및 매칭 시각화 ---
st.markdown("<h2 style='text-align:center;'>📅 2026년 3월 레이드 본부</h2>", unsafe_allow_html=True)
    ...
    for i, day in enumerate(week):
        with cols[i]:
            if day != 0:
                ...
                # 매칭 성공 여부에 따라 CSS 컨테이너 적용
                container_class = "match-gold" if info['is_match'] else ""
                match_label = "\n🏆MATCH!" if info['is_match'] else ""
                
                with st.container():
                    st.markdown(f'<div class="{container_class}">', unsafe_allow_html=True)
                    if st.button(f"{day}\n\n👥{info['count']}{match_label}", key=f"d_{day}"):
...
if not day_df.empty:
    st.markdown(f"### 📊 {sel} 타임라인 " + ("<span style='color:#FFD700;'>[8인 매칭 완료]</span>" if check_8man_match(day_df) else ""), unsafe_allow_html=True)

...
# [1] 요일 시작 설정 (일요일 시작으로 한국식 달력 고정)
calendar.setfirstweekday(calendar.SUNDAY)

# [2] 공대원 명단 (여기서 직접 수정 시 앱에 즉시 반영됩니다)
MEMBER_LIST = ["공대장", "대원1", "대원2", "대원3", "대원4", "대원5", "대원6", "대원7"]

# [3] 시간 설정: 서울(KST) 기준 및 2026년 연도 고정
...
st.markdown("""
    ...
    .stButton > button {
        ...
        color: #E0E0E0 !important; margin-top: 5px; font-size: 0.95rem !important;
        transition: 0.2s;
    }
    ...
    [data-testid="column"] { padding: 0 20px !important; }
    </style>
        ...
        for i, day in enumerate(week):
            with cols[i]:
                if day != 0:
                    ...
                    label = f"{day}\n\n"
                    if info['count'] > 0: label += f"👥{info['count']}"
                    if info['is_match']: label += " 🏆"
